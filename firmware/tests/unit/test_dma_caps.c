 ...
 * rather than any single platform's assumptions. Each case states whether the
...
/* Step a started transfer to DONE, then clear flags. The live engine
 * stays BUSY until time passes; IRQ_CLEAR alone no longer idles it, so
 * every OK start must run before the next case. No-op when idle. */
static void run_to_idle(void) {
    vlab_test_dma_complete();
    hal_write_reg(VLAB_DMA_IRQ_CLEAR, 1U); /* clear DONE/ERROR flags */
}
